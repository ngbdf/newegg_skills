...
```
https://www.newegg.com/p/{ItemNumber}?Item={ItemNumber}&utm_source={platform}&utm_medium=ai_skill&utm_campaign=product-search&utm_content=newegg-product-search
```
...
`utm_campaign` is fixed at `product-search`, `utm_content` at `newegg-product-search` for this skill.
